    ...
    def analyze_fens(self, fens):
        ...
        for fen, (bm, mating_line) in fens:
            ...
            while board.move_stack:
                if bool(board.legal_moves) and self.limitFill and doanalysis:
            ...
            # Ask the engine to clear its transposition table:
            #engine.configure({"Clear Hash": True})

            # Optionally, synchronize to ensure the engine is ready
            #engine.ping()
                ...
                for info in analysis:
                    if "score" in info:
                        ...
                        if m:
                            ...
                            pvstr = " ".join(pv)
                    #print("info: ", info)
            result_fens.append((fen, bm, m, pvstr, retromatesfound, rmpfails ,rmnfails))
...
if __name__ == "__main__":
    ...
    #    logging.basicConfig(filename=args.logFile, filemode="a", level=logging.DEBUG)
    ana = Analyser(args)
    ...
    mates = bestmates = tbwins = rmates = rfailsp = rfailsn = 0
    rmfp = rmfn = 0; 
    issue = {
    ...
    for fen, bestmate, mate, pv , retromatesfound, rmpfails, rmnfails in res:
        rmates += retromatesfound
        rfailsp += int(rmpfails > 0)
        rfailsn += int(rmnfails > 0)
        rmfp += rmpfails
        rmfn += rmnfails
        if mate is None:
            #print(f'failed finding mate on  FEN "{fen}" with bm #{bestmate}, retrograde mate finding failed after {rmpfails + rmnfails} back moves')
            continue
    ...
    print("Found mates on starting pos (FEN):  ", mates)
    print("Best  mates on starting pos (FEN):  ", bestmates)
    print("Found mates  while retrograding:  ", rmates)
    
    print(
    f"Fails/aborts while retrograding on finding M+: {rfailsp:6} "
    f"  after {rmfp / max(1, rfailsp):6.1f} back halfmoves in average"
    )

    print(
    f"Fails/aborts while retrograding on finding M-: {rfailsn:6} "
    f"  after {rmfn / max(1, rfailsn):6.1f} back halfmoves in average"
    )
